...
static std::optional<ShaderType> compileShader(ShaderType type,
...
static unsigned createShader(std::string const &vertexShader,
...
static void checkArgs(int argc, char **argv) {
    if (argc < 3) {
        std::cerr << "Program usage: " << argv[0]
                  << " vertexShader.shdr fragmentShader.shdr\n";
        exit(1);
    }
}
...
static GLFWwindow *getWindow(unsigned width, unsigned height) {
    GLFWwindow *window{
        glfwCreateWindow(width, height, "Window1", nullptr, nullptr)};
...
int main(int argc, char **argv) {
    ...
    unsigned indices[]{// draw in this order
                       0, 1, 2, 2, 3, 0, 0, 4, 3, 1, 5, 2};
    ...
    glVertexAttribPointer(0 /* we only have one attrib*/,
                          2, /* size in elements of the attrib */
                          GL_FLOAT /* type used */, GL_FALSE /* normalized */,
    ...
    glGenBuffers(
        1,
        &indexBufferObj); // generate 1 buffer, put identifier in buffer
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indexBufferObj);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, 12 * sizeof(unsigned), indices,
                 GL_STATIC_DRAW); // give it to OpenGL

    std::optional<std::string> vertexShaderCode{
        graphix::ShaderUtil::readShader(argv[1])};
    std::optional<std::string> fragmentShaderCode{
        graphix::ShaderUtil::readShader(argv[2])};
